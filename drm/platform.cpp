...
PlatformCtx* PlatfromInit( void )
{
   ...
   if ( ctx )
   {
      ...
      if ( !res )
      {
         printf("Eror: PlatformInit: failed to get resources from card (%s)\n", card);
         goto exit;
      }
      ...
      if ( ctx->enc )
      {
         ...
         {
            printf("Error: PlatformInit: unable to determine current mode for connector %p on card %s crtc %p\n", conn, card, ctx->crtc);
         }
      }
      else
      {
